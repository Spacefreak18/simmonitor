...
void* browseloop(SMSettings* sms, char* datadir, char* cachedir)
{
    ...
    DBField driver;
    driver.type = HOEL_COL_TYPE_TEXT;
    driver.offset = offsetof(SessionRowData, driver);
    driver.size = sizeof(unsigned char)*150;
    DBField car;
                ...
                case SESSIONS_SCREEN:
                    ...
                    for(int i=0; i<sessions+1; i++)
                    {
                        ...
                        if (i == 0)
                        {
                            ...
                            mvwaddnstr(bwin1, 3, ((bwiny/width1)*5)+bwiny/width2, "stints", -1);
                            mvwaddnstr(bwin1, 3, ((bwiny/width1)*6)+bwiny/width2, "time", -1);
                ...
                case LAPS_SCREEN:
                    ...
                    for(int i=0; i<lapsresults+1; i++)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                mvwaddnstr(bwin1, 4+i, bwiny/7 - 8, " 1 ", 3);
                                ...
                                {
                                    mvwaddnstr(bwin1, 4+i, bwiny/7 - 8, " 2 ", 3);
                                    LapTime l = hoel_convert_to_simdata_laptime( lapsdb.rows[i-1].time);
