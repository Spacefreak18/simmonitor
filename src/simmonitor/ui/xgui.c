...
void xcallback(uv_timer_t* handle)
{
    ...
    if (!glfwWindowShouldClose(win) && f->uion == true)
    {
        /* input */
        pump_input(&ctx, win);

        /* draw */
        int lrpm = snprintf(NULL, 0, "RPM: %i", simdata->rpms);
        lrpm++;
        char rpmtext[lrpm];
        snprintf(rpmtext, lrpm, "RPM: %i", simdata->rpms);

        {
            ...
            canvas_begin(&ctx, &canvas, 0, 0, 0, width, height, nk_rgb(0,0,0));
            {
                ...
                nk_fill_rect(canvas.painter, nk_rect(20,20,200,200), 5, nk_rgb(0, 0, 0));
                nk_draw_text(canvas.painter, nk_rect(30, 30, 150, 20), rpmtext, lrpm-1, &nfont->handle, nk_rgb(188,174,118), nk_rgb(188,174,118));
